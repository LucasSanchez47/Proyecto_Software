...
class UsuarioModel {
    ...
    public function Actualizar(Usuario $data): void {
        try {
            $params = [
                ...
                $data->getcorreo(),
                $data->getidcargo(),
                $data->getnum_telefono()
            ];

            $sql = "UPDATE usuarios SET 
                    Nombre = ?, Apellido = ?, Direccion = ?, Correo = ?, idCargo = ?  num_telefono = ?";
    ...
    public function Registrar(Usuario $data): void {
        try {
            $sql = "
                INSERT INTO usuarios (Nombre, Apellido, Direccion, Correo, idCargo, Clave, num_telefono) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public function Buscar(string $termino): array {
        try {
            ...
            foreach ($stm->fetchAll(PDO::FETCH_OBJ) as $r) {
                ...
                $usuario->setdireccion($r->Direccion);
                $usuario->setidcargo($r->idCargo);
                $user->setnum_telefono($r->num_telefono);
                $result[] = $usuario;
            }
